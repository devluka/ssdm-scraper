...
```
SUPABASE_URL                 https://xlrbsirplkrbkpmuylwk.supabase.co
SUPABASE_SERVICE_KEY         (service_role 키)
SDM_COLLECTOR_PAT            ssdm-collector 레포 dispatch 권한 PAT
                             (Fine-grained: ssdm-collector contents:write)
```
...
4. (ssdm-collector 측) `parsers/ntis.py` 추가 + PARSER_REGISTRY 등록
...
GitHub Pages 활성화 후:
```
https://devluka.github.io/ssdm-scraper/
```
